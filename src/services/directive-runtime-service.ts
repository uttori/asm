...
export class DirectiveRuntimeService {
  ...
  handlePushNamespace(): void {
    ...
    if (this.host.namespaceNestingEnabled) {
      // Nested namespace mode has two pieces of state: the visible namespace and
      // the path used to reconstruct it after `namespace off`.
  ...
  handlePullNamespace(): void {
    ...
    if (this.host.namespaceNestingEnabled) {
      ...
      // The namespace stack is user-facing assembler state; validate the parsed
      // path instead of trusting JSON.parse's `any` result.
      this.host.namespaceNestingPath = Array.isArray(parsedPath) && parsedPath.every((entry) => typeof entry === "string")
        ? parsedPath
        : [];
    }
  ...
  handleNamespace(params: string[]): void {
    if (params.length >= 2 && params[0].toLowerCase() === "nested") {
      // `namespace nested on/off` changes how later namespace commands compose
      // names, it does not itself enter a namespace.
    ...
    if (params.length === 1 && params[0].toLowerCase() === "off") {
      if (this.host.namespaceNestingEnabled) {
        // In nested mode, turning a namespace off pops one level and rebuilds
        // the visible namespace from the remaining path.
