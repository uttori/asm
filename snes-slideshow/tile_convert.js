...
function process(infile, bpp, tilesize) {
    pilImage = Image.open(infile);
}
    // Non-paletted images are only allowed for Direct Select.
    if (pilImage.mode != 'P') {
        throw Error('image must be paletted');
    if (pilImage.palette.mode != 'RGB') {
        throw Error('image must have an RGB palette');
    }
    if (pilImage.size[0] % tilesize[0] != 0) {
        throw Error('image width must be a multiple of tile width ({})'.format(tilesize[0]));
    if (pilImage.size[1] % tilesize[1] != 0) {
        throw Error('image height must be a multiple of tile height ({})'.format(tilesize[1]));
    }
            ...
            for (j in range(screenSizeTiles[1])) {
                for (i in range(screenSizeTiles[0])) {
                    ...
                    for (p in rawTile) {
                        ...
                        if (!warned && paletteNum !== null && paletteNum != pixelPalette) {
                    ...
                    tilemap.push((tileIndex, paletteNum, flipFlags));
    }
    // Generate palette.
    snesPalette = [];
...
function writeOutput(results, outprefix, bpp, tilesize) {
