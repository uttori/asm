...
import { createMemoryAssemblyFileProvider, MemoryAssemblyFileProvider } from "../src/file-provider.js";
...
test("architecture encoders expose instruction catalogs", (t) => {
  ...
  const spc = assembler.architectureRegistry.getDefinition("spc700")?.encoder.getInstructionCatalog?.() ?? [];
...
test("position lookup resolves references to definitions across files", (t) => {
  const definition: AssemblySymbolDefinition = {
    name: "playerHealth",
    kind: "label",
    location: { file: "/proj/a.asm", line: 2, range: { start: { line: 2, character: 0 }, end: { line: 2, character: 11 } } },
  };
  const reference: AssemblySymbolReference = {
    name: "playerHealth",
    kind: "label",
    location: { file: "/proj/b.asm", line: 5, range: { start: { line: 5, character: 4 }, end: { line: 5, character: 15 } } },
  };
...
test("workspace index analyses SLIDE.SRC and follows includes", (t) => {
  const index = new WorkspaceIndex({ entryPoints: [slideRoot] });
  index.reindex();

  const symbols = index.getSymbols(slideRoot);
  t.true(symbols.some((symbol) => symbol.name === "programStart"), "expected programStart label");

  const edgeTargets = index.getIncludeEdges()
  ...
  const analyzedBasenames = index.getAnalyzedFiles().map((file) => path.basename(file).toLowerCase());
  t.true(analyzedBasenames.includes("compress.src"), "included file should have its own analysis bucket");
