...
test("language-server providers expose navigation, docs, completion, and tokens", t => {
  ...
  t.true(completions.some(item => item.label === "org" && item.kind === CompletionItemKind.Keyword));
  ...
  t.true(
    Array.from(
      { length: semanticTokenData.length / 5 },
      (_, index) => semanticTokenData.slice(index * 5, index * 5 + 5),
