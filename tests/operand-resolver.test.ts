...
test("determineValueLength - handles numeric ranges", (t) => {
  ...
  t.is(resolver.determineValueLength(0x00), 1);
  t.is(resolver.determineValueLength(0xff), 1);
  t.is(resolver.determineValueLength(0x100), 2);
  t.is(resolver.determineValueLength(0xffff), 2);
  t.is(resolver.determineValueLength(0x10000), 3);
  t.is(resolver.determineValueLength(0xffffff), 3);
});
...
test("determineValueLength - is not capped at a three-byte architecture width", (t) => {
  const resolver = createResolver();

  t.is(resolver.determineValueLength(0x1000000), 4);
  t.is(resolver.determineValueLength("$12345678"), 4);
});
...
test("lowerOperand - tolerates whitespace in indexed indirect operands", (t) => {
  ...
  const loweredIndexedIndirect = classify65816Operand(resolver, "(.C8BD, X)");
  t.is(loweredIndexedIndirect.mode, "indexedIndirectX");
  t.is(loweredIndexedIndirect.baseExpression, ".C8BD");

  const loweredIndirectIndexed = classify65816Operand(resolver, "($12), Y");
...
test("lowerOperand - classifies generic indexed Y operands", (t) => {
  ...
  const lowered = classify65816Operand(resolver, "obj.active, Y");
...
test("lowerOperand - keeps resolved 24-bit indexed X operands long", (t) => {
  ...
  const lowered = classify65816Operand(resolver, "_04984F_9879-$02,X");
...
test("lowerOperand - shortens same-bank 24-bit indexed X labels", (t) => {
  ...
  const lowered = classify65816Operand(resolver, "save_file_ptr,x");
...
test("lowerOperand - shortens same-bank resolved indexed X operands", (t) => {
  ...
  const lowered = classify65816Operand(resolver, "_048AD3,X");
...
test("lowerOperand - keeps bank-0 indexed X labels long from another bank", (t) => {
  const tryResolveLabel = sinon
    .stub()
    .withArgs("raphael_mode7_matrix_a_d", false)
    .returns(0x00e954);
  const resolver = createResolver({
    tryResolveLabel,
  ...
  const lowered = classify65816Operand(resolver, "raphael_mode7_matrix_a_d,x");
...
test("lowerOperand - keeps 4-digit hex indexed X absolute even from another bank", (t) => {
  ...
  const lowered = classify65816Operand(resolver, "$E954,x");
...
test("lowerOperand - dp-range labels are not explicit direct page", (t) => {
  ...
  const lowered = classify65816Operand(resolver, "sprite_yspeed");
...
test("lowerOperand - two-digit hex is explicit direct page", (t) => {
  const resolver = createResolver();
  const lowered = classify65816Operand(resolver, "$07");
...
test("lowerOperand - define-expanded two-digit hex is explicit direct page", (t) => {
  ...
  const dp = classify65816Operand(resolver, "!s_spr_wildcard_5_lo_dp");
  t.is(dp.expanded, "$76");
  t.true(dp.explicitDirectPage);

  const indexed = classify65816Operand(resolver, "!s_spr_wildcard_5_lo_dp,x");
...
test("lowerOperand - keeps explicit 6-digit hex long even in the current bank", (t) => {
  ...
  const lowered = classify65816Operand(resolver, "$007972");
...
test("lowerOperand - collapses immediate bitmask expressions to operand width", (t) => {
  const resolveDefines = sinon
    .stub()
    .callsFake((input: string) => input.replaceAll("!x", "$40").replaceAll("!a", "$80"));
  const evaluateMath = sinon.stub().withArgs("$40|$80").returns(0xc0);
  const resolver = createResolver({ resolveDefines, evaluateMath });

  const lowered = classify65816Operand(resolver, "#!x|!a");
