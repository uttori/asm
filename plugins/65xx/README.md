...
| CPU shorthand directives | Supported | `.p02` … `.pm740`. |
| CPU conditionals | Supported | Matching `.ifp*` forms for every plugin-owned CPU. |
| CPU names | Supported | Listed above. 65816-derived CPUs remain SNES-plugin-owned. |
| Legal, undocumented, and vendor instructions | Supported | Includes 6502X, HuC6280, M740, 65CE02/4510, and 45GS02 forms in the declarative catalogs. |

### Expressions & Symbols

| Feature | Status | Notes |
| ------- | ------ | ----- |
| Hex, binary, decimal, and octal literals | Supported | `$`, `%`, decimal, and ca65 `@` octal. |
| Low/high/bank operators | Supported | Unary `<`, `>`, and `^`. |
| Pseudo-functions | Supported | `.defined`, `.lobyte`, `.hibyte`, `.bankbyte`, `.loword`, `.hiword`. |
| Inequality and symbol assignment | Supported | `<>`, `=`, and `:=`. |
| Address-size forcing | Supported | `z:`, `a:`, and `f:` plus `.b`/`.w`. |
| Global, cheap-local, and unnamed labels | Supported | `@name`, `:`, `:+`, `:++`, `:-`, `:--`; file-local isolation across includes. |
| `.scope` / `.proc`, `Scope::Name` | Partial | Flattened into file-local symbol keys. |
| `.export` / `.import` | Partial | Resolves symbols across source files in one session. No relocatable records. |
| `.exportzp`, `.importzp`, `.globalzp`, `.forceimport`, `.autoimport` | Unsupported | Explicit diagnostic. |
...
| `.dword`, `.faraddr`, `.lobytes`, `.hibytes` | Supported | |
| `.res`, `.align`, `.org` | Supported | Operates on the selected flat or iNES image. |
| `.include`, `.incbin` | Supported | `.incbin` uses `file, offset, size`. |
| `.if` / `.ifdef` / `.ifndef` | Supported | Dotted forms lower to the shared engine. |
| `.assert` | Partial | A false assertion stops assembly. Warning/link-time actions are not reproduced. |
| `.undefine` | Supported | |
| `.segment` on `65xx.ca65-raw` | Partial | Names and stack intent; bytes stay in source order. |
| `.segment` on `65xx.nes` | Partial | Selected MEMORY/SEGMENTS load/run/fill; not general ld65. |
| `.constructor`, `.destructor`, `.interruptor`, `.reloc`, `.debuginfo` | Unsupported | Explicit diagnostic. |

### Macros & Repetition

| Feature | Status | Notes |
| ------- | ------ | ----- |
| `.macro` / `.endmacro` and bare invocation | Supported | Comma-separated parameters and `\parameter`. |
| `.repeat` / `.endrepeat` | Supported | Optional named counters. |
| `.local`, `.exitmacro` | Unsupported | Macro-compatibility diagnostic. |
| Token-list/string macros, `.match`, `.xmatch`, `.mid`, `.left`, `.right` | Unsupported | |
